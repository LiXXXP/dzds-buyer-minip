    ...
    <view class="card">
      <view class="title flex row-between">
        <view class="flex col-center">
          ...
          <text class="num">供应：{{ detail.quantity }}{{ detail.unit }}</text>
        </view>
        <view @click="isCollect = !isCollect">
          <image v-if="!isCollect" src="@/static/home/collect.png"></image>
          <image v-else src="@/static/home/collect-select.png"></image>
        </view>
      </view>
      <view class="price flex col-center">
      ...
      <view class="info">
        <view class="flex row-between">
          <text>运输方式</text>
          <text>{{ detail.transportType }}</text>
        </view>
        <view class="flex row-between">
          <text>联系人姓名</text>
        ...
        <view class="flex row-between">
          <text>手机号</text>
          <text>{{ detail.sellerMobile }}</text>
        </view>
        <view class="flex row-between">
          <text>供应商地址</text>
          <text>{{ detail.departureAddress }}</text>
        </view>
      </view>
      <view class="explain">
        <view>详细说明</view>
        <view>{{ detail.memo }}</view>
      </view>
    </view>
    <view class="button flex row-between">
      <view class="dial-btn" @click="showPhone = true">隐私拨打</view>
      <view class="buy-btn" @click="toPay(detail)">立即购买</view>
    </view>
    <!-- 拨打 -->
...
export default {
  ...
  methods: {
    ...
    toPay(detail) {
      uni.navigateTo({
        url: '/pages/pay/index?detail=' + encodeURIComponent(JSON.stringify(detail))
      })
    }
  }
}
