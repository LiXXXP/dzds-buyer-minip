<template>
  <view class="publish">
    ...
    <u--form labelPosition="left" :model="model" :rules="rules" ref="form" labelWidth="220rpx">
      ...
      <u-form-item
        label="收货地址"
        prop="publish.destinationAddress"
        borderBottom
        leftIcon="*"
        :leftIconStyle="{ color: '#F5222D' }"
      ...
      >
        <u--input
          v-model="model.publish.destinationAddress"
          disabled
          inputAlign="right"
          disabledColor="#fff"
          placeholder="请选择"
      ...
      <u-form-item
        label="发货时效"
        prop="publish.limitations"
        borderBottom
        leftIcon="*"
        :leftIconStyle="{ color: '#F5222D' }"
        @click="isShow.time = true"
      >
        <u--input
          v-model="model.publish.limitations"
          disabled
          inputAlign="right"
          disabledColor="#fff"
          placeholder="请选择"
      ...
      <u-form-item label="求购数量" prop="publish.quantity" leftIcon="*" :leftIconStyle="{ color: '#F5222D' }">
        <u--input
      ...
      <u-form-item label="求购价格(元)" prop="publish.price" leftIcon="*" :leftIconStyle="{ color: '#F5222D' }">
        <u--input
      ...
      <u-form-item label="联系人" prop="publish.buyerName" leftIcon="*" :leftIconStyle="{ color: '#F5222D' }">
        <u--input v-model="model.publish.buyerName" border="none" placeholder="请输入" inputAlign="right"></u--input>
      </u-form-item>
      <u-form-item label="手机号" prop="publish.buyerMobile" leftIcon="*" :leftIconStyle="{ color: '#F5222D' }">
        <u--input
          v-model="model.publish.buyerMobile"
          type="number"
          border="none"
          placeholder="请输入"
          inputAlign="right"
    ...
    <u-picker
      ...
      @confirm="
        (e) => {
          model.publish.limitations = e.value
          isShow.time = false
        }
      "
      @cancel="isShow.time = false"
    ></u-picker>
    <!-- 计件单位 -->
    <u-picker
      :show="isShow.unit"
...
export default {
  ...
  methods: {
    ...
    confirmAddr(e) {
      this.model.publish.destinationAddress = [e.value[0].label, e.value[1].label]
    ...
    submit() {
      this.$refs.form
        ...
        .then((res) => {
          ...
          param.limitations = this.model.publish.limitations[0]
          param.unit = this.model.publish.unit[0]
          param.destinationAddress = this.model.publish.destinationAddress[0] + this.model.publish.destinationAddress[1]
